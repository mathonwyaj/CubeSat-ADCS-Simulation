%% Gravity-Gradient Disturbance-Rejection Test
...
commanded_attitude = deg2rad([0; 0; 45]);
...
gravity_torque_history = zeros(number_of_samples, 3);
...
for k = 1:number_of_samples
    ...
    gravity_torque = gravity_gradient_torque( ...
        position_inertial, ...
        state(1:4), ...
        I, ...
        mu);
...
maximum_gravity_torque = ...
    max(abs(gravity_torque_history), [], 1);
...
%% Display results
fprintf('\nGravity-gradient rejection results:\n');
...
fprintf('\nMaximum gravity-gradient torque [N m]:\n');
fprintf('x: %.6e\n', maximum_gravity_torque(1));
fprintf('y: %.6e\n', maximum_gravity_torque(2));
fprintf('z: %.6e\n', maximum_gravity_torque(3));
...
title('Gravity-Gradient Disturbance Rejection');

subplot(3,1,2);
plot(time, gravity_torque_nNm, 'LineWidth', 1.2);
grid on;
ylabel('Gravity torque [nN m]');
